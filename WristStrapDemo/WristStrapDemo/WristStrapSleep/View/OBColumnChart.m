...
//  OBColumnChart
//
//  Created by Obally on 16/5/10.
//  Copyright © 2016年 obally. All rights reserved.
//

#import "OBColumnChart.h"

@interface OBColumnChart ()<CAAnimationDelegate>
...
//        _maxHeight += 4;
    ...
    if (_xShowInfoText.count == _valueArr.count&&_xShowInfoText.count>0) {
        ...
        for (NSInteger i = 0; i<_xShowInfoText.count; i++) {
            ...
            CGFloat wid =  count * _columnWidth + 10;
            ...
            textLayer.frame = CGRectMake( i * (count * _columnWidth + _typeSpace + _columnSpace) + _typeSpace + _originSize.x , CGRectGetHeight(self.frame) - _originSize.y+5,wid, size.height);
    ...
    for (NSInteger i = 0; i<_valueArr.count; i++) {
        ...
        for (NSInteger j = 0; j<arr.count; j++) {
            ...
            itemsView.frame = CGRectMake((i * arr.count + j)*_columnWidth + i*_typeSpace+_originSize.x + _typeSpace + (i + j) * _columnSpace, CGRectGetHeight(self.frame) - _originSize.y-1, _columnWidth, 0);
            
            
//            itemsView.layer.cornerRadius = 4;
            if (_isShowLineChart) {
            ...
            [UIView animateWithDuration:1 animations:^{
                 ...
                 itemsView.frame = CGRectMake((i * arr.count + j)*_columnWidth + i*_typeSpace+_originSize.x + _typeSpace+ (i + j) * _columnSpace, CGRectGetHeight(self.frame) - height - _originSize.y -1, _columnWidth, height);
                [self addCornersWithView:itemsView];
                ...
                if (finished) {
                    ...
                    textLayer.frame = CGRectMake((i * arr.count + j)*_columnWidth + i*_typeSpace+_originSize.x + _typeSpace+(i + j) * _columnSpace, CGRectGetHeight(self.frame) - height - _originSize.y -3 - size.height, _columnWidth, size.height);
...
- (void)addCornersWithView:(UIView *)itemsView
{
    CGRect rect = [itemsView bounds];
...
//    shapeLayer.fillColor = [UIColor redColor].CGColor;
    ...
    itemsView.layer.mask = shapeLayer;
//    [itemsView.layer addSublayer:shapeLayer];
}
